...
func _on_response_updated(new_token: String) -> void:
	if current_ai_npc != null:
		var richtext = current_ai_npc.get_node("CanvasLayer/PanelContainer/VBoxContainer/VBoxContainer2/HBoxContainer/RichTextLabel")
		richtext.text += new_token

func _on_response_finished(_response: String) -> void:
	if current_ai_npc != null:
		var text_edit = current_ai_npc.get_node("CanvasLayer/PanelContainer/VBoxContainer/VBoxContainer2/TextEdit")
